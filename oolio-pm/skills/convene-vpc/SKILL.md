...
4. **Run STORM first.** Invoke the `storm-research` skill (or follow its steps) to discover perspectives, ground claims in real sources, and surface the unknown unknowns. STORM hands you a cited briefing and a set of perspectives so the testing panels argue from evidence, not assumption. Re-enter STORM later if a clash exposes an evidence gap.
